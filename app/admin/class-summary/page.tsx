...
export default async function SubmissionProgressPage({ searchParams }: { searchParams: Promise<{ classId?: string }> }) {
  ...
  const items = await getClassSubmissionProgress(classId);
  ...
  return (
    <main className="desktop-dashboard">
      <header className="page-header">
        ...
        <div className="progress-header-actions">
          <div className="registration-context-bar">
            <RegistrationContextSelector classId={classId} classes={classes.map(({ id, name }) => ({ id, name }))} />
          </div>
      ...
      {classId > 0 && items.length > 0 && (
        <section className="progress-overview" aria-label="班級全部作業概況">
      ...
      {classId <= 0 ? (
        ...
        <div className="empty-state"><i className="bi bi-calendar-check" /><strong>{selectedClass?.name} 還沒有作業項目</strong><span>到「作業登記」新增項目後就會出現在這裡。</span></div>
      ) : (
        <SubmissionProgressBoard
          key={classId}
          className={selectedClass?.name ?? ""}
          headcount={headcount}
          items={items.map((item) => ({ id: item.assignment_id, date: item.date, title: item.title, description: item.description, missingSeats: item.missing_seats }))}
