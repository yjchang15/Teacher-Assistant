...
export default async function ClassSummaryPage({searchParams}:{searchParams:Promise<{date?:string;classId?:string}>}){
  ...
  return <main><header className="mb-3"><h1 className="h3 fw-bold mb-1">全班缺交總表</h1><p className="text-body-secondary mb-0">依作業項目彙整全班缺交座號。</p></header>
    <section className="card mb-4"><AutoSubmitForm className="card-body d-flex align-items-end gap-3"><div><label className="form-label">班級</label><select className="form-select" name="classId" defaultValue={classId}>{classes.map(c=><option key={c.id} value={c.id}>{c.name}</option>)}</select></div><div><label className="form-label">日期</label><input className="form-control" type="date" name="date" max={today} defaultValue={date}/></div></AutoSubmitForm></section>
