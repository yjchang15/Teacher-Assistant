...
export default async function AdminPage({searchParams}:{searchParams:Promise<{start?:string;end?:string;classId?:string;seat?:string}>}){
  ...
  const groups=Object.values(details.reduce<Record<number,{seat:number;rows:typeof details}>>((acc,row)=>{acc[row.seat]??={seat:row.seat,rows:[]};acc[row.seat].rows.push(row);return acc;},{}));
  const base={classId:String(classId),start,end};
  return <main className={onlySeat?"student-report-focus":""}>
    {!onlySeat&&<><header className="mb-3 d-flex justify-content-between align-items-start gap-3"><div><h1 className="h3 fw-bold mb-1">個人缺交列表</h1><p className="text-body-secondary mb-0">每個座號獨立一張表，方便逐一截圖給家長。</p></div><a className="btn btn-success flex-shrink-0" href="/admin/export"><i className="bi bi-file-earmark-excel me-2"/>匯出全部</a></header>
    <section className="card mb-4"><form method="get" className="card-body d-flex align-items-end gap-3"><div><label className="form-label">班級</label><select className="form-select" name="classId" defaultValue={classId}>{classes.map(c=><option key={c.id} value={c.id}>{c.name}</option>)}</select></div><div><label className="form-label">起日</label><input className="form-control" type="date" name="start" max={today} defaultValue={start}/></div><div><label className="form-label">迄日</label><input className="form-control" type="date" name="end" max={today} defaultValue={end}/></div><button className="btn btn-primary">查詢</button></form></section></>}
    {onlySeat>0&&<div className="mb-3 d-print-none"><Link className="btn btn-outline-secondary" href={`/admin?${new URLSearchParams(base)}`}><i className="bi bi-arrow-left me-2"/>返回全部</Link></div>}
    <div className="student-report-list">{groups.length===0?<div className="alert alert-success">目前沒有缺交作業</div>:groups.map(group=><article className="student-report-card" key={group.seat}>
      <header><div><strong>{className}{className.endsWith("班")?"":"班"} {String(group.seat).padStart(2,"0")}號</strong></div>{!onlySeat&&<Link className="btn btn-sm btn-outline-primary d-print-none" href={`/admin?${new URLSearchParams({...base,seat:String(group.seat)})}`}><i className="bi bi-arrows-fullscreen me-1"/>單獨顯示</Link>}</header>
      <table className="table table-bordered align-middle mb-0"><thead><tr><th style={{width:120}}>日期</th><th style={{width:160}}>作業項目</th><th>缺交作業內容</th></tr></thead><tbody>{group.rows.map((row,index)=><tr key={`${row.date}-${row.title}-${index}`}><td className="text-body-secondary">{row.date.replaceAll("-","/")}</td><td className="fw-semibold">{row.title}</td><td>{row.description||"—"}</td></tr>)}</tbody></table>
    </article>)}</div>
  </main>;
}
