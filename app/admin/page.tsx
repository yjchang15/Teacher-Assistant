...
export default async function AdminPage({searchParams}:{searchParams:Promise<{start?:string;end?:string;classId?:string;seat?:string}>}){
  ...
  const groups=Object.values(details.reduce<Record<number,{seat:number;studentNumber:string;name:string;rows:typeof details}>>((acc,row)=>{acc[row.seat]??={seat:row.seat,studentNumber:row.student_number,name:row.student_name,rows:[]};acc[row.seat].rows.push(row);return acc;},{}));
  const base={classId:String(classId),start,end};
  return <main className={onlySeat?"student-report-focus":""}>
    {!onlySeat&&<><header className="mb-3"><h1 className="h3 fw-bold mb-1">缺交統計</h1><p className="text-body-secondary mb-0">每位學生獨立一張表，方便逐一截圖給家長。</p></header>
    ...
    <div className="student-report-list">{groups.length===0?<div className="alert alert-success">目前沒有缺交作業</div>:groups.map(group=><article className="student-report-card" key={group.seat}>
      <header><div><strong>{String(group.seat).padStart(2,"0")} 號</strong>{group.name&&<span>{group.name}</span>}{group.studentNumber&&<small>學號 {group.studentNumber}</small>}</div>{!onlySeat&&<Link className="btn btn-sm btn-outline-primary d-print-none" href={`/admin?${new URLSearchParams({...base,seat:String(group.seat)})}`}><i className="bi bi-arrows-fullscreen me-1"/>單獨顯示</Link>}</header>
      <table className="table table-bordered align-middle mb-0"><thead><tr><th style={{width:80}}>座號</th><th style={{width:220}}>缺交作業</th><th>作業內容</th></tr></thead><tbody>{group.rows.map((row,index)=><tr key={`${row.date}-${row.title}-${index}`}><td className="fw-bold text-center">{group.seat}</td><td><small className="text-body-secondary me-2">{row.date.replaceAll("-","/")}</small>{row.title}</td><td>{row.description||"—"}</td></tr>)}</tbody></table>
    </article>)}</div>
  </main>;
}
